maindir="/ssd/zhanghao/ID_converter/Latest_Data/"
setwd(maindir)
library(tidyr)
...
##===== MGI =====
files <- paste0(maindir,"/",c("MRK_total_list.rpk",
...
merge_file<- Reduce(function(x, y) merge(x, y,by=1, all=TRUE), dl)
merge_file <- merge_file[,c("f1.7","f1.9","f1.11","f1.2","f2.7","f6.14","f6.15","f3.9","f4.6","f5.6","f5.11","f4.7","f4.8","f6.11","f6.12","f6.17")]
merge_file[] <- lapply(merge_file, function(x) as.character(gsub("null", "", x)))
merge_file[is.na(merge_file)] <- ""
merge_file$f5.6 <- ifelse(merge_file$f5.6 == "", merge_file$f3.9, merge_file$f5.6)
merge_file$f5.11 <- ifelse(merge_file$f5.11 == "", merge_file$f4.6, merge_file$f5.11)
merge_file[,c("f6.14","f6.15","f6.11","f6.12","f6.17")] <- 
  lapply(merge_file[,c("f6.14","f6.15","f6.11","f6.12","f6.17")], function(x) as.character(gsub("\\|", " ", x,perl = T)))
merge_file$f2.7 <- paste(merge_file$f2.7,merge_file$f6.14,merge_file$f6.15)
merge_file <- merge_file[,c("f1.7","f1.9","f1.11","f1.2","f2.7","f5.6","f5.11","f4.7","f4.8","f6.12","f6.17")]
mgi_map <- merge_file
colnames(mgi_map) <- c("Gene_Symbol","Gene_name","Gene_type","Chrom","UniprotID","ENTREZID","ENSEMBL_gene","ENSEMBL_rna","ENSEMBL_protein","Nucleotide_rna_Accession","Nucleotide_protein_Accession")
base_name <- c("Gene_Symbol","Gene_name","Gene_type","Chrom","ENTREZID","ENSEMBL_gene")
mgi_map<- rbind.fill(mgi_map[,c(base_name,"UniprotID")] %>% separate_rows(UniprotID,sep = " "),
                     mgi_map[,c(base_name,"ENSEMBL_rna")] %>% separate_rows(ENSEMBL_rna,sep = " "),
                     mgi_map[,c(base_name,"ENSEMBL_protein")] %>% separate_rows(ENSEMBL_protein,sep = " "),
                     mgi_map[,c(base_name,"Nucleotide_rna_Accession")] %>% separate_rows(Nucleotide_rna_Accession,sep = " "),
                     mgi_map[,c(base_name,"Nucleotide_protein_Accession")] %>% separate_rows(Nucleotide_protein_Accession,sep = " ")
)
mgi_map$Chrom[which(mgi_map$Chrom=="UN")] <- "Un"
mgi_map$Chrom[which(mgi_map$Chrom=="XY")] <- "X|Y"
mgi_map[is.na(mgi_map)] <- ""
mgi_map<- unique(mgi_map)

##===== NCBI =====
files <- paste0(maindir,"/",c("mouse_ncbi.gene_info.tab",
                              "mouse_ncbi.gene2accession.tab",
                              "mouse_ncbi.gene2ensembl.tab",
...
ensembl_species<-useMart(biomart = "ensembl",dataset = "mmusculus_gene_ensembl")
...
##===== Uniprot =====
uniprot_file <- paste0(maindir,"/","MOUSE_10090_idmapping_latest.dat")
...
ENTREZID<-keys(org.Mm.eg.db,keytype = "ENTREZID")
all_IDtypes <- c("SYMBOL","GENENAME","ENTREZID","ENSEMBL","ENSEMBLTRANS","ENSEMBLPROT","UNIPROT","REFSEQ")
bioc_map<-AnnotationDbi::select(org.Mm.eg.db, keys=ENTREZID, keytype = "ENTREZID",columns =all_IDtypes)
...
saveRDS(bioc_map,paste0(maindir,"/", "bioc_map.mouse.db.",Sys.Date(),".rds"))
bioc_map_pro <- bioc_map_rna <- NULL


##===== merge database =====
dl <- NULL
df <- rbind.fill(mgi_map,ncbi_map,ensembl_map,uniprot_map)
df[df==""] <- NA
df<- unique(df)
colnames(df)
...
df<- df[!is.na(df$ENTREZID),] %>%
  ...
  fill(Gene_Symbol,Gene_name,Gene_type,Chrom,ENTREZID,ENSEMBL_gene, .direction = "up") %>% as.data.frame()%>%rbind(df[is.na(df$ENTREZID),])
df<- unique(df)
saveRDS(df,paste0(maindir,"/", "mouse.db.",Sys.Date(),".rds"))
